...
{IDENTIFIERS}               {BEGIN(VARorMETHOD);yymore();}
<VARorMETHOD>[\(]           {BEGIN(METHOD);yymore();}
<METHOD>[^\)]*              yymore();
<METHOD>[\)]                {yyless(strstr(yytext,"(")-yytext); BEGIN(INITIAL); return TYPE_method;}
<VARorMETHOD>[^\(]          {yyless(-1); BEGIN(INITIAL); return TYPE_id;}

[/n]                        return TYPE_newline;
[[:blank:][:space:]]        return TYPE_space;
